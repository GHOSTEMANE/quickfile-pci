/* Arquivar — Fase 5: sugestao automatica de pasta + velocidade.
   - Pastas em cache local (localStorage) -> painel abre instantaneo.
   - Token reutilizado na sessao (cache do MSAL/NAA).
   - Sugestao: aprende com o uso (local) e, na 1a vez de cada remetente, descobre via Graph. */
...
let uso = JSON.parse(localStorage.getItem(LS_USO) || "{}"); // {remetente: {folderId: score}}
...
Office.onReady((info) => {
  ...
  bootstrap(false); // auth + refresh + sugestao, em segundo plano
});
...
function renderLista() {
  ...
  pastas.filter((p) => !q || (p.displayName || "").toLowerCase().includes(q))
    .forEach((p) => ul.appendChild(itemPasta(p)));
}
...
async function descobrirSugestao(rem) {
  try {
    const url = "https://graph.microsoft.com/v1.0/me/messages?$top=25&$select=parentFolderId&$search=" +
      encodeURIComponent('"from:' + rem + '"');
    const r = await graphFetch(url);
    if (!r.ok) return;
    const data = await r.json();
    const counts = {};
    (data.value || []).forEach((m) => {
      const f = m.parentFolderId;
      if (pastasById[f] && !ehSistema(pastasById[f].displayName)) counts[f] = (counts[f] || 0) + 1;
    });
...
function aprender(rem, folderId) {
  if (!rem) return;
  uso[rem] = uso[rem] || {};
  uso[rem][folderId] = (uso[rem][folderId] || 0) + 1;
